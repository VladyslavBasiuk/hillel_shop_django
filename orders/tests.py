...
def test_add_discount_order(client, faker, product, discount, login_user):
    ...
    response = client.post(reverse('discount_shopping_cart'), fake_data,
                           follow=True)  # noqa
    assert response.status_code == 200
    assert not response.context_data['order'].discount

    response = client.post(reverse('discount_shopping_cart'), data,
                           follow=True)  # noqa
    assert response.status_code == 200
...
def test_pay_order(client, product, faker, discount, login_user):
    ...
    assert response.status_code == 200
    assert any(template.name == 'orders/successful_pay.html' for template in
               response.templates)  # noqa
