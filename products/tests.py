...
def test_favourites_page(client, faker, login_user, product):
    ...
    assert any(i[0] == reverse('login') + f'?next={url}' for i in response.redirect_chain) # noqa
    assert any(template.name == 'registration/login.html' for template in response.templates) # noqa
...
def test_add_and_delete_favourites(client, faker, product, login_user):
    ...
    response = client.post(reverse('add_or_remove_favorite'), fake_data, follow=True) # noqa
    assert response.status_code == 200
    assert not product in response.context_data['favourites'].products.iterator() # noqa

    response = client.post(reverse('add_or_remove_favorite'), data, follow=True) # noqa
    assert response.status_code == 200
